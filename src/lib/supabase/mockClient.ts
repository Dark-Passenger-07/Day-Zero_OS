  ...
  projects: [
    {
      id: 'proj-1',
      name: 'Day Zero OS',
      description: 'Building the ultimate project-centric operating system for developers, hackers, and creators.',
      status: 'active',
      priority: 'high',
      progress: 68,
      deadline: '2026-08-30',
  ...
  assets: [
    {
      ...
      asset_type: 'image',
      file_url: 'https://images.unsplash.com/photo-1611532736597-de2d4265fba3?auto=format&fit=crop&w=400&q=80',
  ...
  auth: {
    async getSession() {
      ...
      const mockSession = {
        ...
        token_type: 'bearer',
        expires_in: 3600,
        user: { id: 'mock-user-id', email: localStorage.getItem('day_zero_os_mock_email') || 'builder@dayzero.dev' } as User,
      } as Session
      ...
      executePending() {
        if (builder._updates) {
          ...
          if (updatedRow) {
            ...
            let notificationType: 'project' | 'milestone' | 'content' | 'asset' | 'decision' | 'system' = 'system'

            const oldRow = table.find(r => r[idCol] === filterVal)
            ...
            if (newStatus && oldStatus !== newStatus) {
              if (currentTable === 'projects' && newStatus === 'completed') {
                ...
                notificationType = 'project'
              } else if (currentTable === 'project_bugs' && (newStatus === 'fixed' || newStatus === 'closed')) {
            ...
            saveStoredData(db)
            records = records.map(row => row[idCol] === filterVal ? updatedRow : row)
            if (!records.some(row => row[idCol] === filterVal)) {
              records = [updatedRow]
            }
          }
        ...
        if (builder._isDelete) {
          ...
          db[currentTable] = table.filter(row => row[idCol] !== filterVal)
          saveStoredData(db)
