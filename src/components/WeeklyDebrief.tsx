...
export default function WeeklyDebrief() {
  ...
  async function handleAddEntry(section: DebriefSection) {
    ...
    const values = await openForm({
      ...
      fields: [{ name: 'value', label: section.label, type: 'textarea', required: true }],
    })
  ...
  async function handleAddGoal() {
    ...
    const values = await openForm({
      ...
      fields: [{ name: 'value', label: 'Priority Description', type: 'textarea', required: true }],
    })
    if (!values?.value.trim()) return
  ...
  return (
    <div className="h-full overflow-y-auto p-4 sm:p-6 lg:p-9">
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <button
            ...
            style={{
              ...
              padding: '7px',
              cursor: 'pointer',
              color: weekIndex >= debriefs.length - 1 ? 'var(--border)' : 'var(--muted-foreground)',
              display: 'flex',
            }}
          >
            <ChevronLeft size={14} />
          ...
          <button
            ...
            style={{
              ...
              padding: '7px',
              cursor: 'pointer',
              color: weekIndex === 0 ? 'var(--border)' : 'var(--muted-foreground)',
              display: 'flex',
            }}
          >
      ...
      {!current ? (
        ...
        >
          Create your first weekly debrief to start tracking wins, challenges, lessons, and next-week
          priorities.
        </div>
      ) : (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
            {[
              ...
              {
                label: 'Wins',
                value: String(current.wins.length),
                detail: 'captured this week',
                color: 'var(--status-green)',
              },
              {
                label: 'Lessons',
                value: String(current.lessons.length),
                detail: 'future leverage',
                color: 'var(--status-purple)',
              },
              {
                label: 'Priorities',
                value: String(goals.length),
                detail: 'next week goals',
                  ...
                  >
                    <span
                      style={{
                        ...
                        height: '20px',
                        borderRadius: '50%',
                        background: index === 0 ? 'var(--status-blue)' : 'var(--secondary)',
                        border: `1px solid ${index === 0 ? 'var(--status-blue)' : 'var(--border)'}`,
                        display: 'flex',
                        alignItems: 'center',
          ...
          >
            ...
            <div style={{ fontSize: '13px', color: 'var(--muted-foreground)' }}>
              Time metrics are stored in the debrief metrics object and can be expanded after core MVP
            ...
            >
              <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                ...
                <span style={{ fontSize: '12px', color: 'var(--muted-foreground)' }}>
                  Reflection entries: <strong style={{ color: 'var(--foreground)' }}>{totalEntries}</strong>
                </span>
              </div>
            </div>
          </div>
