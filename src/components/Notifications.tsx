...
import { deleteNotification, listNotifications, markAllNotificationsRead, markNotificationRead, type NotificationItem } from '@/features/notifications/services/notifications.service'
  ...
  return (
    <div className="h-full overflow-y-auto p-4 sm:p-6 lg:p-9">
      <div className="flex flex-row justify-between items-start mb-6">
        ...
        <button onClick={() => act(markAllNotificationsRead, 'All notifications marked read.')} style={buttonStyle}><CheckCheck size={13} /> Mark all read</button>
      </div>
      ...
      <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '10px', overflow: 'hidden' }}>
        {items.length === 0 ? (
          <div style={{ padding: '48px', textAlign: 'center', color: 'var(--muted-foreground)', fontSize: '14px' }}>No notifications.</div>
        ) : items.map(item => (
          <div key={item.id} style={{ display: 'flex', gap: '12px', padding: '14px 18px', borderBottom: '1px solid var(--border)', background: item.readAt ? 'transparent' : 'rgba(59,130,246,0.06)' }}>
            <Bell size={15} color={item.readAt ? 'var(--muted-foreground)' : 'var(--status-blue)'} />
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: '13px', fontWeight: 500 }}>{item.title}</div>
              {item.body && <div style={{ fontSize: '12px', color: 'var(--muted-foreground)', marginTop: '3px' }}>{item.body}</div>}
              <div style={{ fontSize: '11px', color: 'var(--muted-foreground)', marginTop: '5px' }}>{item.type} · {new Date(item.createdAt).toLocaleString()}</div>
            </div>
            {!item.readAt && <button onClick={() => act(() => markNotificationRead(item.id), 'Notification marked read.')} style={iconStyle}><CheckCheck size={13} /></button>}
            <button onClick={() => act(() => deleteNotification(item.id), 'Notification deleted.')} style={iconStyle}><Trash2 size={13} /></button>
          </div>
        ))}
      </div>
    </div>
...
const buttonStyle: React.CSSProperties = { display: 'flex', alignItems: 'center', gap: '6px', background: 'var(--secondary)', border: '1px solid var(--border)', borderRadius: '6px', padding: '7px 12px', color: 'var(--secondary-foreground)', fontSize: '12px', cursor: 'pointer', fontFamily: 'inherit' }
const iconStyle: React.CSSProperties = { background: 'none', border: 'none', color: 'var(--muted-foreground)', cursor: 'pointer', padding: '4px', display: 'flex' }

function Notice({ color, text }: { color: string; text: string }) {
  return <div style={{ border: `1px solid ${color}`, color, borderRadius: '8px', padding: '10px 12px', fontSize: '13px', marginBottom: '16px' }}>{text}</div>
}
