...
import { updateKnowledgeEntry, deleteKnowledgeEntry } from '@/features/project-workspace/services/project-workspace.service'
  ...
  const handleEdit = async (item: KnowledgeEntry) => {
    const values = await openForm({
      ...
      fields: [
        ...
        { name: 'category', label: 'Category', type: 'select', value: item.category, options: ['research', 'framework', 'reference', 'personal-note', 'lesson'] },
  ...
  const handleCreate = async () => {
    ...
    const values = await openForm({
      ...
      fields: [
        ...
        { name: 'category', label: 'Category', type: 'select', value: 'research', options: ['research', 'framework', 'reference', 'personal-note', 'lesson'] },
      ...
      <div className="px-4 sm:px-6 lg:px-9 pt-6 sm:pt-9 flex-shrink-0">
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '24px' }}>
          ...
          <button
            ...
            disabled={creating}
            style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'var(--foreground)', color: 'var(--background)', border: 'none', borderRadius: '6px', padding: '9px 16px', fontSize: '13px', fontWeight: 600, cursor: creating ? 'not-allowed' : 'pointer', fontFamily: 'inherit' }}
          >
        ...
        {filtered.length === 0 ? (
          ...
          <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '10px', overflow: 'hidden' }}>
            {filtered.map((item, index) => (
              <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: '14px', padding: '14px 20px', borderBottom: index < filtered.length - 1 ? '1px solid var(--border)' : 'none' }}>
                <span style={{ color: 'var(--muted-foreground)', flexShrink: 0 }}>{categoryConfig[item.category].icon}</span>
                ...
                <div style={{ display: 'flex', gap: '6px' }}>
                  <button onClick={() => handleEdit(item)} style={{ background: 'none', border: 'none', color: 'var(--muted-foreground)', cursor: 'pointer', padding: '4px' }}>
                    <Pencil size={13} />
                  </button>
                  <button onClick={() => handleDelete(item.id)} style={{ background: 'none', border: 'none', color: 'var(--muted-foreground)', cursor: 'pointer', padding: '4px' }}>
                    <Trash2 size={13} />
                  </button>
                </div>
              </div>
