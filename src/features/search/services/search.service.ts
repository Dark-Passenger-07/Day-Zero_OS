...
export type SearchResult = {
  id: string
  type: 'project' | 'knowledge' | 'content' | 'asset' | 'architecture' | 'activity' | 'weekly' | 'setting' | 'command'
  title: string
  subtitle?: string
  path: string
}
...
export async function searchWorkspace(query: string): Promise<SearchResult[]> {
  ...
  const [projects, knowledge, content, assets, architecture, activity, weekly] = await Promise.all([
    supabase.from('projects').select('id, name, description').ilike('name', pattern).is('deleted_at', null).limit(5),
    ...
    supabase.from('architecture_decisions').select('id, project_id, decision, impact').ilike('decision', pattern).limit(5),
    supabase.from('activity_logs').select('id, project_id, action, entity_type').ilike('action', pattern).limit(5),
  ...
  return [
    ...
    ...(weekly.data ?? [])
      .filter(item => `${item.week_start} ${(item.wins ?? []).join(' ')} ${(item.lessons ?? []).join(' ')}`.toLowerCase().includes(trimmed.toLowerCase()))
      .map(item => ({
        id: item.id,
